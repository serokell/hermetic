  ...
  def handle_event(message = %{type: "message", text: text}, _slack, [ytcache]) do
    ...
    if length(matches) > 0 do
      ...
      Slack.Web.Chat.post_message(message.channel, "", %{
        ...
        thread_ts:
          if Map.has_key?(message, :thread_ts) do
            message.thread_ts
          end
      })
    end
  ...
  def issue_attachment(code) do
    ...
    unless ytinfo == "Issue not found." do
      ...
      %{
        ...
        text:
          if Map.has_key?(ytinfo, :description) do
            ytinfo.description.value
          end,
        ...
        fields: [
          ...
          if Map.has_key?(ytinfo, :Assignees) do
            assignees =
              for %{"fullName" => fullName, "value" => username} <- ytinfo."Assignees".value,
                  do: "<#{YT.prefix()}users/#{username}|#{fullName}>"

            %{title: "Assigned To", value: Enum.join(assignees, ","), short: true}
          end
        ]
      }
    end
